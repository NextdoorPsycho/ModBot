package com.volmit.modbot.adapters;

import com.volmit.modbot.util.Core;
import com.volmit.modbot.ModBot;
import com.volmit.modbot.adapters.prefix.Shutdown;
import com.volmit.modbot.util.instance.SkipCommand;
...
//        jda.addEventListener(new TicketMasterButton());
    ...
    private static void registerAllCommands(String packagePath, JDA jda) throws NullPointerException, IOException {
        ...
        new BufferedReader(new InputStreamReader(stream))
                ...
                .map(cmdClass -> {
                    try {
                        ...
                        e.printStackTrace();
                        ModBot.debug("Failed to load command " + cmdClass.getName() + " with empty constructor!");
                    } catch (ArrayIndexOutOfBoundsException e) {
                        e.printStackTrace();
                        ModBot.debug("Failed to load command " + cmdClass.getName() + " due to no constructor being present!");
                    }
                    return null;
                })
                .filter(Objects::nonNull)
        ...
        stream.close();
        ModBot.debug("Loaded " + (loadedCommands.isEmpty() ? "NONE" : String.join(", ", loadedCommands)) + " from package " + packagePath);
    }
    ...
    private static Class<?> getCommandClass(String className, String packageName) {
        try {
            ...
            if (c.isAssignableFrom(ListenerAdapter.class)) {
                ModBot.debug("Unable to load class: " + c.getName() + " because it does not extend ListenerAdapter");
            }
            return c;
